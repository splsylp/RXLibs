 ...
 * @param mapPoint 要转化的mapPoint
 *
 * @partransform数，比方transform&transform, point.x, point.y);来绘制。总之传入的trans可以修正CGPoint的精度不足所造成的标注抖动问题。如果您只是使用现有的QPolygonView或者QPolylineView则不需要考虑这些问题，因为系统已经为您封装好了相关问题的处理。调用该接口的时候，请不要给trans传NULL，如果你不需要考虑精度问题可以直接使用- (CGPoint)pointFromMapPoint:(QMapPoint)mapPoint
 *
 *  @return 相对于overlayView的本地坐标
 */
